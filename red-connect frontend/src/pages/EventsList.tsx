                ...
                <div className="flex items-start gap-6">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-muted-foreground space-y-1">
                      <div>
                        <strong className="text-primary">Date:</strong> {ev.event_date ? new Date(ev.event_date).toLocaleDateString('en-US', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' }) : "TBA"}
                      </div>
                      <div>
                        <strong className="text-primary">Time:</strong> {ev.start_time ? formatTimeTo12Hour(ev.start_time) : "TBA"} {ev.end_time ? `- ${formatTimeTo12Hour(ev.end_time)}` : ""}
                      </div>
                      <div>
                        <strong className="text-primary">Location:</strong> {ev.venue || "TBA"}
                      </div>
                      <div className="mt-2 font-semibold">
                        <strong className="text-primary">Remaining Slots:</strong> <span className={remaining > 0 ? "text-green-600" : "text-red-600"}>{remaining > 0 ? remaining : "Full"}</span>
                      </div>
                    </div>
                  </div>
