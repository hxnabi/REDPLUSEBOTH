...
def list_certificates(
    ...
    # Add donor names to certificates
    for cert in certificates:
        donor = db.query(Donor).filter(Donor.id == cert.donor_id).first()
        if donor:
            cert.donor_name = donor.full_name
...
def download_certificate(
    ...
    """Download certificate as HTML (can be printed to PDF)"""
    ...
    donor = db.query(Donor).filter(Donor.id == certificate.donor_id).first()
    ...
    certificate_data = {
        ...
        'blood_type': certificate.blood_type,
        'blood_units': certificate.blood_units,
        'donation_date': donation.donation_date.strftime('%B %d, %Y') if donation else certificate.issue_date.strftime('%B %d, %Y'),
        'issue_date': certificate.issue_date.strftime('%B %d, %Y'),
        'certificate_number': certificate.certificate_number,
        'issued_by': certificate.issued_by or 'Blood Donation Organization',
    }
    
    # Generate HTML certificate
    html_content = generate_certificate_html(certificate_data)
